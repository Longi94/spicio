...
public class AddFriendInteractorTest {
    ...
    @Mock
    private AddFriendInteractor.Callback mMockedCallback;
    ...
    public void testSuccess() {
        when(mRepository.addFriend(1L, 2L)).thenReturn(true);

        AddFriendInteractorImpl interactor = new AddFriendInteractorImpl(
                mApp, 1L, 2L, mMockedCallback
        );
        interactor.run();

        verify(mRepository).addFriend(1L, 2L);
        verifyNoMoreInteractions(mRepository);
        verify(mMockedCallback).onAddFriendFinish();
    }

    @Test
    public void testFail() {
        when(mRepository.addFriend(1L, 2L)).thenReturn(false);
        ...
        verify(mRepository).addFriend(1L, 2L);
        verifyNoMoreInteractions(mRepository);
        verify(mMockedCallback).onAddFriendFail();
    }
}
